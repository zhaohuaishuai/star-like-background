String convertAssToAdvancedLyric(
  String assLyric, {
  String title = '',
  String artist = '',
  String album = '',
...
String? _parseDialogueLine(String line) {
  ...
  // 确定最终的字列表：归一化各字时长使其总和等于行总时长。
  // flutter_lyric 的 handleHighlight 用各字 duration 作为 TweenSequence 权重，
  // 但动画总时长是行的完整持续时间。若各字时长之和不等于行总时长，
  // TweenSequence 按比例分配时间会导致每个字的渐变动画速度不准确。
  final totalCharDuration =
      parsedData.fold<int>(0, (sum, item) => sum + item.duration);
  final outputChars = (totalCharDuration > 0 && totalCharDuration != duration)
      ? _normalizeCharDurations(
          parsedData, startTimeMs, duration, totalCharDuration)
      : parsedData;

  // 先添加完整歌词
  final fullLyric = outputChars
      .map((item) => '${item.char} (${item.startTime},${item.duration})')
      .join('');
...
List<_KaraokeChar> _parseKaraokeTags(String text, int startTimeMs) {
  ...
  return tagCharPairs
      .map((pair) => _KaraokeChar(pair.char, pair.startTime, pair.duration))
      .toList();
}
...
/// 归一化各字时长，使其总和等于行总时长（行级别 duration）。
/// 按比例缩放每个字的时长，最后一字吸收舍入误差以确保总时长精确匹配。
List<_KaraokeChar> _normalizeCharDurations(List<_KaraokeChar> chars,
    int lineStartMs, int lineDuration, int totalCharDuration) {
  final scale = lineDuration / totalCharDuration;
  var cumulativeStart = lineStartMs;
  final result = <_KaraokeChar>[];
  for (var i = 0; i < chars.length; i++) {
    final item = chars[i];
    // 最后一个字填满剩余时间，避免多次四舍五入累积误差
    final newDuration = (i == chars.length - 1)
        ? lineDuration - (cumulativeStart - lineStartMs)
        : (item.duration * scale).round();
    result.add(_KaraokeChar(item.char, cumulativeStart, newDuration));
